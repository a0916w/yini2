...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Widget build(BuildContext context) {
    final list = _list;
    return Scaffold(
      appBar: AppBar(title: const Text('观看记录'), actions: [
        if (list != null && list.isNotEmpty)
          TextButton(
            onPressed: () async { await WatchHistory.clear(); if (mounted) setState(() => _list = []); },
            child: Text('清空', style: TextStyle(color: C.ink3, fontSize: 13)),
          ),
      ]),
      body: list == null
          ? const Center(child: CircularProgressIndicator(color: C.brand))
          : list.isEmpty
              ? Center(child: Text('还没有观看记录', style: TextStyle(color: C.ink3)))
              : ListView.separated(
